...
export default function Source({
  ...
  selectedItem: string;
  sourceListAction: SourceListReducerActions;
  ...
  useEffect(() => {
    ...
    console.log("Source: ", selectedItem);
  }, [selectedItem]);
  ...
  async function handleSaveSource() {
    ...
    if (source._id === "0") {
      try {
        ...
        sourceListAction.remove("0");
        sourceListAction.add({ _id: data.insertedId, name: source.name });
    ...
    // if the source is not new, update it
      ...
      try {
        ...
        sourceListAction.update({ _id: source._id, name: source.name });
